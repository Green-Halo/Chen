...
class RunnerConfig:
    ...
    def before_experiment(self) -> None:
        ...
        imdb_subset = imdb.train_test_split(test_size=0.001)['test']
        sst2_subset = sst2.train_test_split(test_size=0.001)['test'] 

        mrpc_subset = mrpc.train_test_split(test_size=0.01)['test']
        qqp_subset = qqp.train_test_split(test_size=0.001)['test']   
         
        wnli_subset = wnli.train_test_split(test_size=0.1)['test']
        rte_subset = rte.train_test_split(test_size=0.01)['test']
    ...
    def start_run(self, context: RunnerContext) -> None:
        ...
        # Check if dataset1 is None
        if dataset1 is not None:
            output.console_log(f"Running first dataset for task: {task_name}")
            inference_time1, gpu_energy1, accuracy1, gpu_utilization1, memory_usage1 = self.run_experiment(model, tokenizer, dataset1, quantization_type, task_name)
        ...
        # Check if dataset2 is None
        if dataset2 is not None:
            output.console_log(f"Running second dataset for task: {task_name}")
            inference_time2, gpu_energy2, accuracy2, gpu_utilization2, memory_usage2 = self.run_experiment(model, tokenizer, dataset2, quantization_type, task_name)
        ...
        # Record averages of both datasets
        context.run_data = {
    ...
    def load_model(self, quantization_type):
        ...
        if quantization_type == "32-bit":
            ...
            tokenizer = AutoTokenizer.from_pretrained("alokabhishek/Meta-Llama-3-8B-Instruct-bnb-8bit", padding_side='left')
            model = AutoModelForCausalLM.from_pretrained("alokabhishek/Meta-Llama-3-8B-Instruct-bnb-8bit",device_map="auto")
    

            #tokenizer = AutoTokenizer.from_pretrained("SweatyCrayfish/llama-3-8b-quantized")
            #model = AutoModelForCausalLM.from_pretrained("PrunaAI/SweatyCrayfish-llama-3-8b-quantized-bnb-8bit-smashed", device_map="auto")
            #tokenizer = AutoTokenizer.from_pretrained("GreenHalo/Llama-3.1-8B-Instruct-GPTQ-8bit", padding_side='left')
            #model = AutoModelForCausalLM.from_pretrained("GreenHalo/Llama-3.1-8B-Instruct-GPTQ-8bit",device_map="auto",trust_remote_code=True)

       
        elif quantization_type == "4-bit":
            tokenizer = AutoTokenizer.from_pretrained("alokabhishek/Meta-Llama-3-8B-Instruct-bnb-4bit", padding_side='left')
            model = AutoModelForCausalLM.from_pretrained("alokabhishek/Meta-Llama-3-8B-Instruct-bnb-4bit",device_map="auto")
    
            #tokenizer = AutoTokenizer.from_pretrained("astronomer/Llama-3-8B-Instruct-GPTQ-4-Bit", padding_side='left')
            #model = AutoModelForCausalLM.from_pretrained("astronomer/Llama-3-8B-Instruct-GPTQ-4-Bit",device_map="auto")
 
            #tokenizer = AutoTokenizer.from_pretrained("SweatyCrayfish/llama-3-8b-quantized")
            #model = AutoModelForCausalLM.from_pretrained("PrunaAI/SweatyCrayfish-llama-3-8b-quantized-bnb-4bit-smashed", device_map="auto")
            #tokenizer = AutoTokenizer.from_pretrained("GreenHalo/Llama-3.1-8B-Instruct-GPTQ-4bit", padding_side='left')
            #model = AutoModelForCausalLM.from_pretrained("GreenHalo/Llama-3.1-8B-Instruct-GPTQ-4bit",device_map="auto")
    ...
    def run_experiment(self, model, tokenizer, dataset, quantization_type, task_name, batch_size=1):
        ...
        for inputs_list, labels in self.batch_process_input(dataset, task_name, batch_size, tokenizer):
            ...
            inputs = tokenizer(inputs_texts, return_tensors="pt", padding=True, truncation=True, max_length=256).to('cuda')
            ...
            for i, (generated_text, correct_choice) in enumerate(zip(generated_texts, labels)):

                output.console_log(f"Generated text for sample {i}: {generated_text}")
            ...
            # Measure performance 
            gpu_energy, gpu_utilization, memory_usage = self.measure_gpu_metrics(inference_time)
        ...
        if valid_text_count == 0:
            output.console_log("No valid samples were processed.")
            return None

        accuracy = correct_predictions / valid_text_count
        return (
            total_inference_time / valid_text_count,
            total_gpu_energy / valid_text_count,
            accuracy,
            total_gpu_utilization / valid_text_count,
            total_memory_usage / valid_text_count
        )
    ...
    def measure_inference(self, model, tokenizer, input_text):
        #output.console_log(f"Starting inference for input text.")
        inputs = tokenizer(input_text, return_tensors="pt").to('cuda')
        torch.cuda.synchronize()  
        start_time = time.perf_counter()
        #outputs = model.generate(inputs["input_ids"], max_length=100, attention_mask=inputs['attention_mask'], num_beams=1)
        outputs = model.generate(inputs["input_ids"], max_new_tokens=50, attention_mask=inputs['attention_mask'], num_beams=1)
        torch.cuda.synchronize() 

        end_time = time.perf_counter()
        inference_time = end_time - start_time
        num_tokens = outputs.shape[1]
        token_latency = (inference_time / num_tokens) * 1000
        #output.console_log(f"Inference completed. Time: {inference_time}s, Token latency: {token_latency}ms.")
        return inference_time, token_latency

    def measure_gpu_metrics(self, inference_time):
        powers = []
        utilizations = []
        memory_usages = []
        start_time = time.time()
        while time.time() - start_time < inference_time:
            powers.append(self.get_gpu_power())
            utilizations.append(self.get_gpu_utilization())
